...
# Compare remaining_compute_mode across SLO variants for a single task (default: GSM8K).
# SLO variants: tight_ttfb (TTFB SLO /2), tight_tpob (TPOB SLO /2)
...
TASKS=(${TASKS:-gsm8k})
SLO_VARIANTS=(${SLO_VARIANTS:-tight_all})
...
REMAINING_COMPUTE_MODES=(${REMAINING_COMPUTE_MODES:-bellman zero worst oracle}) # bellman zero worst oracle warm_start warm_start_continue

# Per-task request rates
RATES_GSM8K=(${RATES_GSM8K:-8 8.5 9})
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${SCRATCH_ROOT}/dlm_bellman_slo_change}"
PORT="${PORT:-30010}"
...
# Precompute base SLO values in seconds from the reference JSON (called once).
...
# Per-variant SLO tables (overwritten each variant iteration).
declare -A _V_STRICT_TTFB _V_STRICT_TPOB _V_RELEASE_TTFB _V_RELEASE_TPOB

# Compute variant SLO values for all tasks and store in _V_* arrays.
_compute_variant_slos() {
    local _variant="$1"
    local _task
    for _task in "${TASKS[@]}"; do
        local _st="${_SLO_STRICT_TTFB_S[$_task]}"
        local _sp="${_SLO_STRICT_TPOB_S[$_task]}"
        local _rt="${_SLO_RELEASE_TTFB_S[$_task]}"
        local _rp="${_SLO_RELEASE_TPOB_S[$_task]}"
        case "$_variant" in
            tight_ttfb)
                _V_STRICT_TTFB[$_task]=$(python3 -c "print($_st / 2)")
                _V_STRICT_TPOB[$_task]="$_sp"
                _V_RELEASE_TTFB[$_task]=$(python3 -c "print($_rt / 2)")
                _V_RELEASE_TPOB[$_task]="$_rp"
                ;;
            tight_tpob)
                _V_STRICT_TTFB[$_task]="$_st"
                _V_STRICT_TPOB[$_task]=$(python3 -c "print($_sp / 2)")
                _V_RELEASE_TTFB[$_task]="$_rt"
                _V_RELEASE_TPOB[$_task]=$(python3 -c "print($_rp / 2)")
                ;;
            tight_all)
                _V_STRICT_TTFB[$_task]=$(python3 -c "print($_st / 4)")
                _V_STRICT_TPOB[$_task]=$(python3 -c "print($_sp / 4)")
                _V_RELEASE_TTFB[$_task]=$(python3 -c "print($_rt / 4)")
                _V_RELEASE_TPOB[$_task]=$(python3 -c "print($_rp / 4)")
                ;;
            tight_all3)
                _V_STRICT_TTFB[$_task]=$(python3 -c "print($_st / 3)")
                _V_STRICT_TPOB[$_task]=$(python3 -c "print($_sp / 3)")
                _V_RELEASE_TTFB[$_task]=$(python3 -c "print($_rt / 3)")
                _V_RELEASE_TPOB[$_task]=$(python3 -c "print($_rp / 3)")
                ;;
...
# Write a variant-specific SLO config JSON (used by slorate + plot scripts).
_write_variant_slo_config() {
    local _variant="$1" _outfile="$2"
    python3 -c "
import json
d = json.load(open('${SLO_CONFIG_PATH}'))
variant = '${_variant}'
for task in list(d.keys()):
    if variant == 'tight_ttfb':
        d[task]['strict']['ttfb_ms'] /= 2
        d[task]['relaxed']['ttfb_ms'] /= 2
    elif variant == 'tight_tpob':
        d[task]['strict']['tpob_ms'] /= 2
        d[task]['relaxed']['tpob_ms'] /= 2
    elif variant == 'tight_all':
        d[task]['strict']['ttfb_ms'] /= 4
        d[task]['strict']['tpob_ms'] /= 4
        d[task]['relaxed']['ttfb_ms'] /= 4
        d[task]['relaxed']['tpob_ms'] /= 4
    elif variant == 'tight_all3':
        d[task]['strict']['ttfb_ms'] /= 3
        d[task]['strict']['tpob_ms'] /= 3
        d[task]['relaxed']['ttfb_ms'] /= 3
        d[task]['relaxed']['tpob_ms'] /= 3
with open('${_outfile}', 'w') as f:
    json.dump(d, f, indent=2)
print('[slo_variant] ' + variant + ' config -> ${_outfile}')
"
}
...
# Validate SLO config and precompute base tables
...
# Build ordered modes (same for all variants):
#   bellman always first, oracle always last, warm_start before oracle
...
# ============================================================
# Main loop: SLO variant → RC mode → task → rate
# ============================================================
for SLO_VARIANT in "${SLO_VARIANTS[@]}"; do
    VARIANT_ROOT="${OUTPUT_ROOT}/slo_${SLO_VARIANT}"
    mkdir -p "${VARIANT_ROOT}"
    SERVER_LOG="${VARIANT_ROOT}/server_log.txt"

    echo
    echo "############################################################"
    echo "SLO Variant: ${SLO_VARIANT}"
    echo "############################################################"

    # Compute variant SLO values and write variant SLO config JSON
    _compute_variant_slos "${SLO_VARIANT}"
    VARIANT_SLO_CONFIG="${VARIANT_ROOT}/slo_config.json"
    _write_variant_slo_config "${SLO_VARIANT}" "${VARIANT_SLO_CONFIG}"

    # Reset per-variant oracle/warmstart tables
    unset _ORACLE_TABLES _WARMSTART_TABLES
    declare -A _ORACLE_TABLES _WARMSTART_TABLES
    _ORACLE_TABLE_DIR="${VARIANT_ROOT}/oracle_tables"
    ...
    for RC_MODE in "${_ordered_modes[@]}"; do
        for TASK in "${TASKS[@]}"; do
            ...
            _strict_ttfb="${_V_STRICT_TTFB["${TASK}"]:-}"
            _strict_tpob="${_V_STRICT_TPOB["${TASK}"]:-}"
            _release_ttfb="${_V_RELEASE_TTFB["${TASK}"]:-}"
            _release_tpob="${_V_RELEASE_TPOB["${TASK}"]:-}"

            for RATE in "${_task_rate_arr[@]}"; do
                OUT_DIR="${VARIANT_ROOT}/rc_mode_${RC_MODE}/request_rate_${RATE}/${TASK}"
                mkdir -p "${OUT_DIR}"

                echo
                echo "============================================================"
                echo "slo_variant=${SLO_VARIANT}  rc_mode=${RC_MODE}  rate=${RATE}  task=${TASK}"
                ...
                echo "===== slo=${SLO_VARIANT} rc_mode=${RC_MODE} rate=${RATE} task=${TASK} =====" >> "${SERVER_LOG}"
    ...
    echo
    echo "============================================================"
    echo "DLM SLO rates — slo_variant=${SLO_VARIANT}"
    echo "============================================================"
    ...
    for RC_MODE in "${_ordered_modes[@]}"; do
        for TASK in "${TASKS[@]}"; do
            read -ra _task_rate_arr <<< "$(_task_rates "${TASK}")"
            echo
            echo "rc_mode=${RC_MODE}  task=${TASK}  slo_variant=${SLO_VARIANT}"
            echo "============================================================"
            ...
            for RATE in "${_task_rate_arr[@]}"; do
                ...
                python test/dlm_slorate.py \
                    --latency-dir "${VARIANT_ROOT}/rc_mode_${RC_MODE}/request_rate_${RATE}/${TASK}" \
                    --tasks "${TASK}" \
                    --slo-config "${VARIANT_SLO_CONFIG}" \
                    --output-json "${VARIANT_ROOT}/rc_mode_${RC_MODE}/slo_rates_${TASK}_rate${RATE}.json"
            done

            _latency_dirs=()
            for RATE in "${_task_rate_arr[@]}"; do
                _latency_dirs+=("${VARIANT_ROOT}/rc_mode_${RC_MODE}/request_rate_${RATE}/${TASK}")
            done
            ...
            python test/dlm_slorate.py \
                ...
                --tasks "${TASK}" \
                --slo-config "${VARIANT_SLO_CONFIG}" \
                --output-json "${VARIANT_ROOT}/rc_mode_${RC_MODE}/slo_rates_${TASK}.json"
        done
    done

    echo
    echo "============================================================"
    echo "Throughput Summary — slo_variant=${SLO_VARIANT}"
    echo "============================================================"

    _TASK_RATES_FILE="${VARIANT_ROOT}/task_rates.json"
...
PYEOF
    ...
    echo
    echo "============================================================"
    echo "Scatter & SLO attainment plots — slo_variant=${SLO_VARIANT}"
    echo "============================================================"
...
echo "  tight_ttfb:  ${OUTPUT_ROOT}/slo_tight_ttfb/"
echo "  tight_tpob:  ${OUTPUT_ROOT}/slo_tight_tpob/"
